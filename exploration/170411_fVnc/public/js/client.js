...
function Client(screen) {
  this._screen = screen;
  this._scaleFactor = 1;
  this._socket = io('http://localhost:8090');
...
Client.prototype._addHandlers = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    self._socket.on('init', function (config) {
      ...
      }, 500); //set timer to send array of coordinates to server every half a second
    });
...
/*Client.prototype._sendTouchPos = function (){
  var self = this;
  self._socket.emit('touch', {data: this._touchPos});
  self._touchPos = [];
}*/
